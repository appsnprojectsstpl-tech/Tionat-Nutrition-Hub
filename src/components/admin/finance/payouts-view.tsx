...
export function PayoutsView() {
    ...
    const ledgerEntriesQuery = useMemoFirebase(() => {
        if (!firestore) return null;
        return query(collection(firestore, 'ledger_entries'), orderBy('timestamp', 'desc'), limit(50));
    }, [firestore]);

    const { data: ledgerEntries, isLoading: isLoadingLedger } = useCollection<LedgerEntry>(ledgerEntriesQuery);
