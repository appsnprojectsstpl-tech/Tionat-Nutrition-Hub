...
export function ProductClient({ initialProduct }: ProductClientProps) {
    ...
    // Fallback if no initialProduct passed (e.g. pure client nav if not supported?)
    // But page is Server Component now.
    ...
    const [product, setProduct] = useState<(Product & { stock: number }) | null>(
        initialProduct ? { ...initialProduct, stock: 0 } : null
    );
    const [isLoading, setIsLoading] = useState(!initialProduct);
    const [quantity, setQuantity] = useState(1);
    const [error, setError] = useState<string | null>(null);

    // Fetch or Update Stock logic
    useEffect(() => {
        ...
        const fetchProductData = async () => {
            ...
            // If we have initialProduct, we just need to check STOCK.
            // If we lack initialProduct, we fetch everything.
    ...
    }, [firestore, slug, selectedWarehouse]); // Depend on slug just in case
    ...
    // Copying the render JSX from previous file
    return (
                ...
                <div className="flex flex-col gap-4 relative z-10 -mt-6 md:mt-0 bg-background rounded-t-3xl md:rounded-none px-4 md:px-0 pt-6 md:pt-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] md:shadow-none">
                    <div className="flex items-start justify-between">
                        <div>
                            ...
                            <p className="text-sm text-muted-foreground mt-1">{product.weight} {product.unit} (Mock)</p>
                        </div>
